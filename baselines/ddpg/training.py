...
def train(env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor, critic,
    ...
    with U.single_threaded_session() as sess:
        ...
        for epoch in range(nb_epochs):
            ...
            if eval_env is not None:
                combined_stats['test/return'] = np.mean(eval_episode_rewards)
                combined_stats['test/return_history'] = np.mean(eval_episode_rewards_history)
                combined_stats['test/Q'] = np.mean(eval_qs)
                combined_stats['test/episodes'] = len(eval_episode_rewards)
                combined_stats['test/success_rate'] = np.mean(success_history)
            def as_scalar(x):
